...
const TravelChallengesPinned = ({ challengeItems, challengesSection, cta_section_70 }: { challengeItems: any[]; challengesSection: any; cta_section_70: any }) => {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mt-6 lg:mt-8 relative rounded-2xl border border-white/10 bg-white/[0.03] grid grid-cols-2 lg:grid-cols-5 divide-x divide-white/10 overflow-hidden">
          {challengeItems.map((it, i) => {
...
const TravelTechShowcase = ({ techStack, technologiesSection, cta_section_111 }: { techStack: any[]; technologiesSection: any; cta_section_111: any }) => {
  const [active, setActive] = useState(0);
...
const challengesSection = pageData?.build_exceptional_travel_hospitality_experiences_through_digital_innovation;
const solutionsSection = pageData?.travel_hospitality_solutions_we_deliver;
...
const engagementModelsSection = pageData?.flexible_engagement_models;
const buildFutureSection = pageData?.build_the_future_of_travel_with_confidence;
...
const heroBlocks = heroBanner?.blocks?.map((block: any) => ({
  ...
  return (
    ...
    <SeoTags
      ...
      {/* ================= CHALLENGES ================= */}
      <TravelChallengesPinned challengeItems={challengeItems} challengesSection={challengesSection} cta_section_70={cta_section_70} />
      ...
      <section className="py-8 lg:py-12 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {/* Flagship card */}
          {solutions.length > 0 && (
            <div className="relative rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-accent/20 mb-5">
              <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-accent via-primary/50 to-accent opacity-40 hover:opacity-80 transition-opacity duration-500" />
              <div className="absolute inset-[1px] rounded-2xl bg-gradient-to-br from-[#0f172a] to-[#0a1222]" />
              <div
                className="absolute inset-0 opacity-0 hover:opacity-100 transition-opacity duration-700 pointer-events-none"
                style={{ background: "radial-gradient(circle at 50% 0%, rgba(92, 200, 220, 0.15) 0%, transparent 60%)" }}
              />
              <div className="relative h-full flex flex-col md:flex-row md:items-center gap-6 md:gap-8 border border-white/5 rounded-2xl p-6 lg:p-8">
                <div className="flex-shrink-0">
                  <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center border border-accent/30">
                    <DynamicIcon name={solutions[0].icon} className="w-8 h-8 lg:w-10 lg:h-10 text-accent" />
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg lg:text-2xl font-semibold text-foreground leading-tight text-left mb-3">
                    {solutions[0].title}
                  </h3>
                  <p className="text-muted-foreground leading-relaxed text-left text-sm lg:text-base">
                    {solutions[0].desc}
                  </p>
                </div>
                <div className="hidden md:flex flex-shrink-0 items-center justify-center">
                  <div className="w-24 h-24 lg:w-32 lg:h-32 rounded-full border border-accent/20 flex items-center justify-center bg-gradient-to-br from-accent/5 to-transparent">
                    <div className="w-16 h-16 lg:w-24 lg:h-24 rounded-full border border-accent/30 flex items-center justify-center">
                      <DynamicIcon name={solutions[0].icon} className="w-8 h-8 lg:w-12 lg:h-12 text-accent/70" />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-12">
            {solutions.slice(1).map((s) => (
              <div
                key={s.title}
                className="group relative rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl hover:shadow-accent/20 h-full"
              >
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-accent via-primary/50 to-accent opacity-40 group-hover:opacity-80 transition-opacity duration-500" />
                <div className="absolute inset-[1px] rounded-2xl bg-gradient-to-br from-[#0f172a] to-[#0a1222]" />
                <div
                  className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"
                  style={{ background: "radial-gradient(circle at 50% 0%, rgba(92, 200, 220, 0.15) 0%, transparent 60%)" }}
                />
                <div className="relative h-full flex flex-col gap-4 border border-white/5 rounded-2xl p-6">
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 flex items-center justify-center border border-accent/30 group-hover:border-accent group-hover:from-primary/30 group-hover:to-accent/30 transition-all duration-500">
                      <DynamicIcon name={s.icon} className="w-6 h-6 text-accent group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base font-semibold text-foreground leading-tight text-left group-hover:text-accent transition-colors duration-300">
                        {s.title}
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="absolute top-0 right-0 w-20 h-20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
                    <div className="absolute top-3 right-3 w-8 h-[1px] bg-gradient-to-r from-transparent to-accent" />
                    <div className="absolute top-3 right-3 w-[1px] h-8 bg-gradient-to-b from-accent to-transparent" />
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ================= TECHNOLOGIES ================= */}
      <TravelTechShowcase techStack={techStack} technologiesSection={technologiesSection} cta_section_111={cta_section_111} />
      ...
      {/* ================= CONSULTING & DEVELOPMENT PROCESS ================= */}
      <section className="py-8 lg:py-12 relative overflow-hidden" style={{ background: "linear-gradient(180deg, hsl(222 47% 6%) 0%, hsl(222 47% 8%) 50%, hsl(222 47% 6%) 100%)" }}>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-5xl mb-8 mx-auto text-center">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground tracking-tight text-center mb-5" dangerouslySetInnerHTML={{ __html: addClassToSpan(pageData?.our_consulting_development_process?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
          </div>
          <div className="relative max-w-6xl mx-auto group">
            <div aria-hidden="true" className="absolute -inset-6 rounded-[2rem] blur-3xl opacity-60 group-hover:opacity-90 transition-opacity duration-700" style={{ background: "radial-gradient(ellipse at 30% 20%, rgba(0,119,182,0.35), transparent 60%), radial-gradient(ellipse at 70% 80%, rgba(95,194,227,0.28), transparent 60%)" }} />
            <div className="relative rounded-2xl p-[1.5px] bg-gradient-to-br from-accent/60 via-primary/30 to-accent/60">
              <div className="relative rounded-2xl overflow-hidden bg-background/60 backdrop-blur-sm" style={{ boxShadow: "0 30px 80px rgba(0,0,0,0.55), 0 0 60px rgba(95,194,227,0.10)" }}>
                <img
                  src={pageData?.our_consulting_development_process?.image?.url || ""}
                  alt={pageData?.our_consulting_development_process?.image?.alt || ""}
                  className="w-full h-auto block transition-transform duration-700 group-hover:scale-[1.01]"
                  width={pageData?.our_consulting_development_process?.image?.width || 1774}
                  height={pageData?.our_consulting_development_process?.image?.height || 887}
                  loading="eager" decoding="async"
                />
                <div aria-hidden="true" className="pointer-events-none absolute inset-0">
                  <div className="absolute top-4 left-4 w-10 h-10 border-t-2 border-l-2 border-accent/70 rounded-tl-xl" />
                  <div className="absolute top-4 right-4 w-10 h-10 border-t-2 border-r-2 border-accent/70 rounded-tr-xl" />
                  <div className="absolute bottom-4 left-4 w-10 h-10 border-b-2 border-l-2 border-accent/70 rounded-bl-xl" />
                  <div className="absolute bottom-4 right-4 w-10 h-10 border-b-2 border-r-2 border-accent/70 rounded-br-xl" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-[0_8px_40px_rgba(0,0,0,0.55)]">
            <img
              ...
              className="w-full h-auto object-cover"
              width={whyChooseSection?.image?.width || 1774}
              height={whyChooseSection?.image?.height || 887}
              loading="eager"
              decoding="async"
            />
          </div>
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-5xl mb-8 mx-auto text-center">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground tracking-tight text-center mb-5" dangerouslySetInnerHTML={{ __html: addClassToSpan(engagementModelsSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
          </div>

          <div className="relative max-w-6xl mx-auto group mb-10">
            <div
              aria-hidden="true"
              className="absolute -inset-4 rounded-[2rem] blur-3xl opacity-50 group-hover:opacity-80 transition-opacity duration-700"
              style={{
                background:
                  "radial-gradient(ellipse at 30% 20%, rgba(0,119,182,0.35), transparent 60%), radial-gradient(ellipse at 70% 80%, rgba(95,194,227,0.28), transparent 60%)",
              }}
            />
            <div className="relative rounded-2xl p-[1.5px] bg-gradient-to-br from-accent/60 via-primary/30 to-accent/60">
              <div
                className="relative rounded-2xl overflow-hidden bg-background/60 backdrop-blur-sm"
                style={{
                  boxShadow:
                    "0 30px 80px rgba(0,0,0,0.55), 0 0 60px rgba(95,194,227,0.10)",
                }}
              >
                <img
                  src={engagementModelsSection?.image?.url || ""}
                  alt={engagementModelsSection?.image?.alt || ""}
                  className="w-full h-auto block transition-transform duration-700 group-hover:scale-[1.01]"
                  loading="eager"
                  decoding="async"
                  width={engagementModelsSection?.image?.width || 1690}
                  height={engagementModelsSection?.image?.height || 931}
                />
                <div aria-hidden="true" className="pointer-events-none absolute inset-0">
                  <div className="absolute top-4 left-4 w-10 h-10 border-t-2 border-l-2 border-accent/70 rounded-tl-xl" />
                  <div className="absolute top-4 right-4 w-10 h-10 border-t-2 border-r-2 border-accent/70 rounded-tr-xl" />
            ...
            <div
              aria-hidden="true"
              className="absolute left-1/2 -translate-x-1/2 -bottom-3 h-[2px] w-40 rounded-full"
              style={{
                background:
                  "linear-gradient(90deg, transparent, rgba(95,194,227,0.9), transparent)",
                boxShadow: "0 0 20px rgba(95,194,227,0.7)",
              }}
            />
          </div>
