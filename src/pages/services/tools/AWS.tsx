...
const AWSServicesTabs = ({ services }: { services: ServiceItem[] }) => {
  ...
  return (
    <div>
      ...
      <div
        ref={tabsRef}
        role="tablist"
        aria-label="AWS Consulting Services"
      ...
      <div
        ...
        role="tabpanel"
        id={`aws-panel-${active}`}
        aria-labelledby={`aws-tab-${active}`}
...
const AWS = () => {
  ...
  const heroContent = pageData?.build_a_secure_scalable_and_future_ready_with_aws_cloud_environment;
  const servicesContent = pageData?.end_to_end_aws_consulting_services;
  const securityContent = pageData?.aws_security_governance_and_operational_excellence;
  const growthContent = pageData?.cloud_solutions_that_support_your_business_as_it_grows;
  ...
  const servicesTabs = (servicesContent?.tabs || []).map((tab: any) => ({
    ...
    desc: tab?.content || "",
    image: tab?.image?.url || "",
    image_label: tab?.image_text || "",
  }));
  
  const securityCards = (securityContent?.cards || []).map((card: any) => ({ 
    icon: card?.icon, 
    title: card?.title || "", 
    content: card?.content || "" 
  }));
  
  const growthCards = (growthContent?.cards || []).map((card: any) => ({ 
    icon: card?.icon, 
    title: card?.title || "", 
    content: card?.content || "" 
  }));
  
  const whyUsCards = (whyUsContent?.cards || []).map((card: any) => ({ 
    icon: card?.icon, 
    title: card?.title || "", 
    content: card?.content || "" 
  }));
    ...
    <>
      ...
      >
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          ...
          <AWSServicesTabs services={servicesTabs} />
      ...
      <section
        ref={setRef("security")}
        className={`relative py-8 lg:py-12 overflow-hidden transition-all duration-700 ${visible.security ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
      ...
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="max-w-6xl mx-auto text-center mb-10 lg:mb-14">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-[1.1] mb-5 text-center" dangerouslySetInnerHTML={{ __html: addClassToSpan(securityContent?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
            <p className="text-muted-foreground text-base sm:text-lg leading-[1.65] text-center">
              {securityContent?.paragraph || ""}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 auto-rows-fr">
            {securityCards.map((w: any, i: number) => (
      ...
      <section
        ref={setRef("growth")}
        className={`relative py-8 lg:py-12 overflow-hidden transition-all duration-700 ${visible.growth ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        style={{ background: "linear-gradient(180deg, hsl(222 47% 7%) 0%, hsl(222 47% 5%) 100%)" }}
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="max-w-6xl mx-auto text-center mb-10 lg:mb-14">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold leading-[1.1] mb-5 text-center" dangerouslySetInnerHTML={{ __html: addClassToSpan(growthContent?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
            <p className="text-muted-foreground text-base sm:text-lg leading-[1.65] text-center">
              {growthContent?.paragraph || ""}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 auto-rows-fr">
            {growthCards.map((w: any, i: number) => (
              <div key={i} className="group relative p-5 lg:p-7 rounded-2xl flex flex-col h-full transition-all duration-300 hover:-translate-y-1" style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(148,163,184,0.12)" }}>
                <div className="flex items-center gap-4 mb-3">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: "linear-gradient(135deg, rgba(95,194,227,0.18), rgba(0,119,182,0.18))", border: "1px solid rgba(95,194,227,0.25)" }}>
                    <DynamicIcon name={w.icon} className="w-6 h-6 text-accent" />
