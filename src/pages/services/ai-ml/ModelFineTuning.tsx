...
const ModelFineTuning = () => {
  ...
  const techniquesSection = pageData?.ai_cards_image_section || {};
  ...
  const techniques = techniquesSection?.cards?.map((card: any) => ({
    icon: card.icon,
    title: card.title,
    desc: card.content,
    image: card.image || "",
  ...
  const technologies = technologiesSection?.cards?.map((card: any) => ({
    ...
    title: card.title,
    desc: card.content,
    bottomLabel1: card.bottom_label_1 || "",
    bottomLabel2: card.bottom_label_2 || "",
    bottomLabel3: card.bottom_label_3 || "",
    bottomLabel4: card.bottom_label_4 || "",
    bottomLabel5: card.bottom_label_5 || "",
  })) || [];
  ...
  return (
          ...
          <div className="grid lg:grid-cols-[5fr_6fr] gap-8 lg:gap-12 items-center">
            <div
              className={`relative transition-all duration-700 ease-out ${
                inViewMap.hero ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"
              }`}
            >
              <div
            ...
            >
              ...
              <div className="space-y-4 text-sm sm:text-base text-muted-foreground leading-relaxed mb-6 text-left">
                <p dangerouslySetInnerHTML={{ __html: heroBanner?.paragraph || "" }} />
              </div>
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Link to={heroBanner?.button_talk_expert_url || ""}>
                  <Button
                    size="lg"
                    variant="outline"
                    className="px-8 py-6 rounded-lg border-accent/30 hover:border-accent/60 hover:bg-accent/5"
                  >
                    {heroBanner?.button_talk_expert_text || ""}
                  </Button>
                </Link>
              </div>
            </div>
    ...
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(whyMattersSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
                ...
                >
                  ...
                  <div className={`relative p-6 sm:p-8 lg:p-10 flex flex-col justify-center ${reverse ? "lg:order-1" : ""}`}>
                    <div className="flex items-center gap-4 mb-3">
                      ...
                      >
                        <DynamicIcon name={typeof g.icon === 'string' ? g.icon : ''} className="w-6 h-6 text-accent" />
                      </div>
      ...
      <section
        ...
        aria-label="Our AI Model Fine-Tuning Services"
      >
        {/* ambient background */}
        <div
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10 max-w-[1500px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-5 lg:gap-6">
            {services.map((s, i) => {
              // desktop spans
              const spans = [
                "lg:col-span-6",  // 01 LLM (featured)
                "lg:col-span-6",  // 02 Domain (featured)
                "lg:col-span-3",  // 03
                "lg:col-span-3",  // 04
                "lg:col-span-3",  // 05
                "lg:col-span-3",  // 06
                "lg:col-span-8",  // 07 Deployment (wide featured)
                "lg:col-span-4",  // 08
              ];
      ...
      {/* ======= TECHNIQUES ======= */}
      <section
        ref={setRef("techniques")}
        className="relative py-10 lg:py-14 overflow-hidden"
        style={{ background: "linear-gradient(180deg, hsl(220 50% 7%) 0%, hsl(222 47% 5%) 100%)" }}
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="text-center mb-12">
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(techniquesSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
      <p className="text-muted-foreground text-sm sm:text-base max-w-3xl mx-auto leading-relaxed">{techniquesSection?.paragraph}</p>
  </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6 max-w-[1500px] mx-auto">
            {techniques.map((t, i) => {
              const isVisible = !!inViewMap.techniques;
              return (
                <article
                  key={i}
                  className={`group relative overflow-hidden rounded-2xl transition-all duration-500 ease-out hover:-translate-y-1 flex flex-col h-full ${
                    isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
                  }`}
                  style={{
                    transitionDelay: `${i * 60}ms`,
                    background:
                      "linear-gradient(160deg, rgba(11,30,54,0.85) 0%, rgba(6,15,30,0.9) 100%)",
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "radial-gradient(circle at 20% 0%, rgba(95,194,227,0.18) 0%, transparent 55%)",
                    }}
                  />
                  <div
                    className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                    style={{ boxShadow: "inset 0 0 0 1px rgba(95,194,227,0.45), 0 0 30px rgba(95,194,227,0.12)" }}
                  />
                  {/* Grid texture */}
                  <div
                  ...
                  {/* Visual - fixed height */}
                  <div
                    className="relative flex items-center justify-center h-[130px] px-4 flex-shrink-0"
                    style={{
                      background:
                        "linear-gradient(135deg, rgba(15,35,65,0.6) 0%, rgba(6,15,30,0.4) 100%)",
                      borderBottom: "1px solid rgba(95,194,227,0.08)",
                    }}
                  >
                    <div
                      className="w-full max-w-[180px] transition-transform duration-500 group-hover:scale-[1.05]"
                      aria-label={`${t.title} illustration`}
                      dangerouslySetInnerHTML={{ __html: t.image || '' }}
                    />
                  </div>
                  ...
                  <div className="relative z-10 flex-1 flex flex-col p-5">
                    <div className="flex items-center gap-2.5 mb-2.5">
                      <div
                        className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 transition-transform duration-500 group-hover:rotate-[-6deg]"
                        style={{
                          background: "rgba(95,194,227,0.08)",
                          border: "1px solid rgba(95,194,227,0.25)",
                        }}
                      >
                        <DynamicIcon name={typeof t.icon === 'string' ? t.icon : ''} className="w-4.5 h-4.5 text-accent" />
                      </div>
      ...
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
           <div className="text-center mb-12">
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(challengesSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
      <p className="text-muted-foreground text-sm sm:text-base max-w-3xl mx-auto leading-relaxed">{challengesSection?.paragraph}</p>
  </div>
          <ChallengeHub items={challenges} />
    ...
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(industriesSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
      ...
      <InlineCTA
        title={pageData?.cta_section_320?.heading || ""}
        sub={pageData?.cta_section_320?.paragraph || ""}
        btn={pageData?.cta_section_320?.button_text || ""}
        btnUrl={pageData?.cta_section_320?.button_url || ""}
      />
      ...
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="text-center mb-12">
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(technologiesSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
      ...
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
           <div className="text-center mb-12">
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(benefitsSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
      <p className="text-muted-foreground text-sm sm:text-base max-w-3xl mx-auto leading-relaxed">{benefitsSection?.paragraph}</p>
  </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-5 lg:gap-6">
            {benefits.map((b, i) => {
              // Row 1: 3 cards × col-span-4 ; Row 2: 4 cards × col-span-3
              const span = i < 3 ? "lg:col-span-4" : "lg:col-span-3";
              return (
                <div
                  key={i}
                  className={`group relative ${span} rounded-2xl p-7 flex flex-col items-start justify-start overflow-hidden transition-all duration-500 min-h-0 ${
                    inViewMap.benefits ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
                  }`}
                  style={{
                    background:
                      "linear-gradient(160deg, rgba(15,23,42,0.85) 0%, rgba(11,17,32,0.9) 100%)",
                    border: "1px solid rgba(148,163,184,0.14)",
                    boxShadow: "0 4px 24px rgba(0,0,0,0.35)",
                    transitionDelay: `${i * 70}ms`,
                  }}
                  onMouseEnter={(e) => {
                    e.currentTarget.style.border = "1px solid rgba(95,194,227,0.45)";
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(90deg, transparent 0%, #5FC2E3 30%, #0077B6 70%, transparent 100%)",
                    }}
                  />
                  {/* Corner glow */}
                  <div
                  ...
                  <div className="relative z-10 flex items-center gap-4 mb-5">
                    <div
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, rgba(95,194,227,0.18) 0%, rgba(0,119,182,0.12) 100%)",
                        border: "1px solid rgba(95,194,227,0.28)",
                      }}
                    >
                      <DynamicIcon name={typeof b.icon === 'string' ? b.icon : ''} className="w-6 h-6 text-accent" />
                    </div>
      ...
      >
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="text-center mb-12">
    <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" dangerouslySetInnerHTML={{ __html: addClassToSpan(whyChooseSection?.heading, "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent") }} />
      <p className="text-muted-foreground text-sm sm:text-base max-w-3xl mx-auto leading-relaxed">{whyChooseSection?.paragraph}</p>
          ...
          <WhyUsBlueprint
            ...
            centerTagline="Strategy to Fine-Tuned Value"
            lifecycleLabel={whyChoose?.[0]?.bottomText || ""}
            ariaLabel="AI fine-tuning capabilities"
          />
          ...
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-start">
            ...
            >
              ...
              <ul className="space-y-3 mb-6">
                {contactSection?.lists?.map((listItem: any, i: number) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-foreground/85">
                    <CheckCircle className="w-5 h-5 text-accent flex-shrink-0 mt-0.5" />
                    <span>{typeof listItem === 'string' ? listItem : listItem.list}</span>
                  </li>
                )) || []}
              </ul>
