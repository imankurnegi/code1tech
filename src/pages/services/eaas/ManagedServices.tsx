...
const ManagedServices = () => {
  ...
  const introSection =
    pageData?.managed_it_services_for_your_business_growth ?? {};
  const introBlocks = Array.isArray(introSection.right_blocks)
    ? introSection.right_blocks
    : [];
  const whyNeedSection =
    pageData?.engineer_first_approach_on_demand_engineers ?? {};
  const whyNeedBlocks = Array.isArray(whyNeedSection.blocks)
    ? whyNeedSection.blocks
    : [];
  const compareSection = pageData?.traditional_it_vs_managed_it ?? {};
  const compareCards = Array.isArray(compareSection.cards)
    ? compareSection.cards
    : [];
  ...
  const processSection =
    pageData?.simple_3_step_process_to_hire_managed_it_services ?? {};
  const processCards = Array.isArray(processSection.cards)
    ? processSection.cards
    : [];
  const whyChooseSection = pageData?.why_smbs_enterprises_choose_us ?? {};
  const whyChooseCards = Array.isArray(whyChooseSection.cards)
    ? whyChooseSection.cards
    : [];
  const industriesSection = pageData?.industries_we_support ?? {};
  const industryCards = Array.isArray(industriesSection.cards)
    ? industriesSection.cards
    : [];
  const plansSection =
    pageData?.choose_a_managed_it_service_plan_that_works_for_you ?? {};
  ...
  const getStartedLists = Array.isArray(getStarted.lists)
    ? getStarted.lists
    : [];
  const growthSection = pageData?.your_growth_deserves_better_it ?? {};
  const growthCards = Array.isArray(growthSection.cards)
    ? growthSection.cards
    : [];
  const contactSection = pageData?.services_get_started_section ?? {};
  const contactLists = Array.isArray(contactSection.lists)
    ? contactSection.lists
    : [];
    ...
    <div>
      ...
      >
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-8 lg:pt-12">
          <div className="grid lg:grid-cols-[5fr_6fr] gap-8 lg:gap-12 items-center">
            ...
            <div>
              <div className="relative">
                ...
                {/* Floating badges */}
                {Array.isArray(banner.floating_badge_fields) &&
                banner.floating_badge_fields.length > 0 ? (
                  <>
                    <div
                      className="absolute right-4 top-1/4 p-3 rounded-xl backdrop-blur-xl hidden sm:block"
                      style={{
                    ...
                    <div
                      className="absolute left-4 bottom-1/4 p-3 rounded-xl backdrop-blur-xl hidden sm:block"
                      style={{
                        background: "rgba(10, 15, 30, 0.85)",
                        border: "1px solid rgba(95, 194, 227, 0.2)",
                        boxShadow: "0 8px 24px rgba(0, 0, 0, 0.4)",
            ...
            <div>
              ...
              {banner.banner_description ? (
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: (banner.banner_description || "").replace(
                      /text-foreground\/90/g,
                ...
                <div className="space-y-4 text-sm sm:text-base text-muted-foreground leading-relaxed mb-6 text-left">
                  <p className="text-muted-foreground font-medium">
                    Stop Fighting IT Hiring Struggles. Let's Build a More
                    Resilient, Secure, and Scalable Business.
                  </p>
                  <p>
                    Running a business means relying on technology for
                    everything from operations and communication to customer
                    experience, revenue generation, and security. When your
                    technology fails, growth slows, productivity suffers, and
                    scaling opportunities are lost.
                  </p>
                  <p className="text-muted-foreground font-medium">
                    Code1 Tech Systems can help you deal with all your
                    struggles!
                  </p>
                </div>
              )}
          ...
          {stats.length > 0 ? (
            <div
              className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 p-4 sm:p-6 rounded-2xl backdrop-blur-2xl"
              style={{
                background:
            ...
            <div className="grid lg:grid-cols-2 gap-10 lg:gap-14 items-stretch max-w-6xl mx-auto">
              {introSection.left_content && (
                <div className="order-1 flex flex-col justify-center">
                  <div
                    className="text-muted-foreground leading-relaxed text-left text-base space-y-4"
                    dangerouslySetInnerHTML={{
                      __html: introSection.left_content,
                    }}
                  />
                </div>
              )}
              {introBlocks.length > 0 && (
                <div className="relative order-2 flex">
                  <div
                    className="relative w-full rounded-2xl p-6 border border-border/40 bg-gradient-to-br from-[#0b1a2e]/80 to-[#0a1422]/80 backdrop-blur-sm flex flex-col"
                    style={{
                      boxShadow:
                        "0 20px 60px rgba(0, 0, 0, 0.5), 0 0 40px rgba(0, 78, 158, 0.1)",
                    }}
                  >
                    <div className="grid grid-cols-2 gap-4 flex-1">
                      {introBlocks.map((block: any, i: number) => (
                        <div
                          key={i}
                          className="flex flex-col items-start justify-between gap-3 p-5 rounded-xl border border-border/30 min-h-[120px]"
                          style={{
                            background:
                              i % 2 === 0
                                ? "linear-gradient(135deg, rgba(95,194,227,0.12), rgba(95,194,227,0.03))"
                                : "linear-gradient(135deg, rgba(0,119,182,0.12), rgba(0,119,182,0.03))",
                          }}
                        >
                          <div className="w-11 h-11 rounded-lg flex items-center justify-center bg-background/40 border border-accent/20">
                            <DynamicIcon
                              name={block.icon}
                              className="w-5 h-5 text-[#5FC2E3]"
                            />
                          </div>
                          <span className="text-foreground/90 text-sm font-medium leading-tight">
                            {block.text}
                          </span>
                        </div>
                      ))}
                    </div>
                    {(introSection.right_bottom_percentage ||
                      introSection.right_bottom_text) && (
                      <div className="mt-4 flex items-center justify-between px-4 py-3 rounded-lg border border-accent/20 bg-background/30">
                        ...
                        <span className="text-base font-mono font-semibold text-[#5FC2E3]">
                          {introSection.right_bottom_percentage}
                        </span>
                      </div>
                    )}
            ...
            {whyNeedBlocks.length > 0 && (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {whyNeedBlocks.map((b: any, i: number) => (
                  <Card
                    key={i}
                    className="bg-card/40 border-border/30 backdrop-blur-sm hover:border-primary/50 transition-all duration-500"
                  >
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid lg:grid-cols-2 gap-6 max-w-6xl mx-auto">
              {compareCards.map((c: any, i: number) => {
                const isManaged = c.icon?.includes("check") || i === 1;
                return (
                  <div
                    key={i}
                    ...
                    <div className="flex items-center gap-3 mb-5">
                      <div
                        className={`w-10 h-10 rounded-lg ${
                          isManaged ? "bg-accent/15" : "bg-destructive/15"
                        } flex items-center justify-center`}
                      >
                        <DynamicIcon
                          name={c.icon}
                          className={`w-5 h-5 ${isManaged ? "text-accent" : "text-destructive"}`}
                        />
                      </div>
            ...
            {coreCards.length > 0 && (
              <div className="grid lg:grid-cols-2 gap-6 max-w-6xl mx-auto">
                {coreCards.map((s: any, i: number) => (
                  <Card
                    key={i}
                    className={`bg-card/40 border-border/30 backdrop-blur-sm hover:border-primary/50 transition-all duration-500 ${i === coreCards.length - 1 && coreCards.length % 2 === 1 ? "lg:col-span-2" : ""}`}
                  >
                    <CardContent className="p-6">
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: (s.content || s.description || "").replace(
                            /text-foreground\/80/g,
                            "text-muted-foreground",
                          ),
                        }}
                      />
              ...
              <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4 max-w-6xl mx-auto">
                {processCards.map((item: any, i: number) => (
                  <div key={i} className="group relative">
                    ...
                    <div className="relative flex flex-col bg-[hsl(222,47%,7%)] border border-white/10 rounded-2xl p-6 sm:p-8 h-full transition-transform duration-300 group-hover:-translate-y-2">
                      <div className="flex items-center gap-4 mb-4 justify-start">
                        <div className="flex items-center justify-center w-16 h-16 rounded-xl bg-gradient-to-br from-[#5FC2E3]/10 to-[#0077B6]/10 border border-[#5FC2E3]/20 flex-shrink-0">
                          <DynamicIcon
                            name={item.icon}
                            className="w-8 h-8 text-[#5FC2E3]"
                          />
                        </div>
                        <h3
                          className="text-xl font-bold text-foreground leading-tight text-left"
                          dangerouslySetInnerHTML={{
                            __html: item.title ?? item.title ?? "",
                          }}
                        ></h3>
                      </div>
                      <div className="font-mono text-[10px] tracking-widest text-[#5FC2E3]/60 uppercase mb-2 text-center">
                        {item.label}
                      </div>
                      <p
                        className="text-muted-foreground text-center text-sm leading-relaxed"
                        dangerouslySetInnerHTML={{
                          __html: item.content ?? item.paragraph ?? "",
                        }}
                      />
                    </div>
                  </div>
                      ...
                      <div className="relative p-6 sm:p-7 flex flex-col gap-4">
                        <div className="flex items-center gap-4">
                          <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-[#5FC2E3]/20 to-[#0077B6]/15 border border-[#5FC2E3]/30 flex-shrink-0">
                            <DynamicIcon
                              name={b.icon}
                              className="w-6 h-6 text-[#5FC2E3]"
                            />
                          </div>
                          <h3
                            className={`font-bold text-foreground leading-tight ${featured ? "text-lg sm:text-xl" : "text-base sm:text-lg"}`}
                          >
                            {b.title}
                          </h3>
                        </div>
                        <p
                          className="text-muted-foreground text-sm text-left leading-relaxed"
                          dangerouslySetInnerHTML={{
                            __html: b.content ?? b.paragraph ?? "",
                          }}
                        />
                      </div>
                    </article>
            ...
            {industryCards.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 max-w-5xl mx-auto">
                {industryCards.map((it: any, i: number) => (
                  <div
                    key={i}
                    className="p-5 rounded-xl border border-border/30 bg-card/30 backdrop-blur-sm hover:border-primary/40 hover:-translate-y-1 transition-all duration-500 text-left"
                  >
                    ...
                    <p className="text-muted-foreground text-xs leading-snug text-center">
            ...
            {planCards.length > 0 && (
              <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                {planCards.map((p: any, i: number) => (
                  <Card
                    key={i}
                    className={`bg-card/40 border-border/30 backdrop-blur-sm hover:border-primary/50 transition-all duration-500 ${i === 1 ? "lg:scale-105 border-accent/40" : ""}`}
                  >
                    <CardContent className="p-6">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          ...
                          {p.tag && (
                            <span className="inline-block text-xs text-accent bg-accent/10 border border-accent/20 px-2 py-0.5 rounded-full mt-1.5">
                              {p.tag}
                            </span>
                          )}
                        </div>
                      ...
                      <p
                        className="text-muted-foreground text-sm text-left leading-relaxed"
                        dangerouslySetInnerHTML={{
                          __html: p.content ?? p.description ?? "",
                        }}
                      />
                    </CardContent>
                  </Card>
        ...
        >
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            ...
            <div
              ...
              style={{
                background:
                  "radial-gradient(circle, rgba(0,119,182,0.10) 0%, transparent 70%)",
                filter: "blur(40px)",
              }}
            />
          </div>
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="max-w-6xl mx-auto">
              <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-6 lg:gap-0 items-stretch">
                {/* Left — manifesto */}
                <div
                  className="relative rounded-2xl lg:rounded-r-none p-8 sm:p-10 lg:p-12 overflow-hidden flex flex-col justify-between"
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(95,194,227,0.06) 0%, rgba(10,15,30,0.85) 45%, rgba(0,119,182,0.08) 100%)",
                    border: "1px solid rgba(95,194,227,0.18)",
                    borderRight: "1px dashed rgba(95,194,227,0.25)",
                  }}
                >
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "repeating-linear-gradient(135deg, #5FC2E3 0px, #5FC2E3 1px, transparent 1px, transparent 14px)",
                    }}
                  />
                    ...
                    {takeItOff.sub_heading && (
                      <div className="relative pl-5 mb-5 text-left">
                        <span
                          ...
                          style={{
                            background:
                              "linear-gradient(180deg, #5FC2E3, #0077B6)",
                          }}
                        />
                        <p className="text-foreground/95 font-semibold text-base sm:text-lg leading-snug">
                          {takeItOff.sub_heading}
                        </p>
                      </div>
                    )}
                    {takeItOff.content && (
                      <p className="text-muted-foreground text-sm sm:text-base leading-relaxed text-left max-w-md">
                        {takeItOff.content}
                      </p>
                    )}
                  </div>
                  ...
                  {Array.isArray(takeItOff.bottom_label) &&
                    takeItOff.bottom_label.length > 0 && (
                      <div
                        className="relative z-10 mt-8 pt-6 flex items-center gap-6 text-[11px] font-mono uppercase tracking-wider text-muted-foreground/70"
                        style={{
                ...
                {/* Right — get started */}
                <div
                  className="relative rounded-2xl lg:rounded-l-none p-8 sm:p-10 lg:p-12 overflow-hidden flex flex-col"
                  style={{
                    background:
                      "linear-gradient(160deg, #0E1525 0%, #0B1220 55%, #12102A 100%)",
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    <div className="flex items-start justify-between gap-4 mb-7">
                      <div className="text-left">
                        {getStarted.top_label && (
                          <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground/70 mb-2">
                            {getStarted.top_label}
                          </p>
                        )}
                        {getStarted.heading ? (
                        ...
                        {getStartedLists.map((item: any, idx: number) => (
                          ...
                          >
                            {idx < getStartedLists.length - 1 && (
                              <span
                                className="absolute left-[14px] top-8 bottom-[-1rem] w-px"
                                style={{
                                  background:
                                    "linear-gradient(180deg, rgba(95,194,227,0.4), rgba(95,194,227,0.05))",
                                }}
                              />
                            )}
                    ...
                    <div
                      ...
                      style={{ borderTop: "1px dashed rgba(148,163,184,0.18)" }}
                    >
                      <Link to={getStarted.button_url ?? ""} className="block">
                        <Button
                          variant="hero"
                          size="lg"
                          className="group w-full text-sm sm:text-base"
                        >
              ...
              {(growthSection.heading || growthCards.length > 0) && (
                <div
                  className="mt-8 relative rounded-2xl p-8 sm:p-10 overflow-hidden"
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(0,119,182,0.10) 0%, rgba(10,15,30,0.85) 50%, rgba(95,194,227,0.08) 100%)",
                    border: "1px solid rgba(95,194,227,0.18)",
                  }}
                >
                  <div className="relative z-10">
                      ...
                      <div className="text-left">
                        {growthSection.heading && (
                          <h3
                            ...
                            dangerouslySetInnerHTML={{
                              __html: addClassToSpan(
                                growthSection.heading.replace(/&amp;/g, "&"),
                                "bg-gradient-to-r from-[#5FC2E3] to-[#0077B6] bg-clip-text text-transparent",
                              ),
                            }}
                          />
                      ...
                      {growthCards.length > 0 && (
                        <div className="grid sm:grid-cols-2 gap-4">
                          {growthCards.map((gc: any, i: number) => (
                            <div
                              key={i}
                              className={`relative p-5 rounded-xl text-left group hover:-translate-y-0.5 transition-transform flex items-center gap-4 ${
                                i === growthCards.length - 1 && growthCards.length % 2 === 1 ? "sm:col-span-2" : ""
                              }`}
                              style={{
                                background:
                            ...
                            >
                              ...
                              <p className="text-[#94c3d1] text-sm font-medium leading-snug">
                                {gc.title ?? gc.label}
                              </p>
                            </div>
                          ))}
                    ...
                    {growthSection.bottom_text && (
                      ...
                      >
                        <p className="text-muted-foreground font-semibold text-base sm:text-lg text-left">
                          {growthSection.bottom_text}
                        </p>
                      </div>
                    )}
            ...
            <div>
              ...
              "
                ...
                <p className="text-muted-foreground mb-6 text-left">
                  Our managed IT services allow customers to mitigate
                  operational burden, improve security, and maintain reliable
                  technology functionality throughout their entire environment.
                </p>
              )}
