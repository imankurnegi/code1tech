...
interface CaseStudyData {
  ...
  content: {
    ...
    tab_3_sixth_heading: string;
    tab_3_sixth_content: string;
    tab_3_sixth_image: {
      url: string;
      alt: string;
      width: number;
...
interface PowerBiCaseStudyProps {
  data: CaseStudyData;
}
...
const mapArchitectureFlow = (blocks: CaseStudyData["content"]["tab_3_second_blocks"]) => {
  return blocks.map((block, index) => ({
    ...
    label: block.title,
    tone: (index === 1 || index === 4) ? "mint" as const : undefined,
  }));
};
...
const extractDocumentationList = (htmlContent: string): string[] => {
  const ulRegex = /<ul[^>]*>.*?<\/ul>/gs;
  const ulMatch = htmlContent.match(ulRegex);
  if (!ulMatch) return [];
  
  const liRegex = /<li[^>]*>.*?<\/li>/gs;
  const liMatches = ulMatch[0].match(liRegex) || [];
...
const mapIntegrationPoints = (blocks: CaseStudyData["content"]["tab_3_first_blocks"]) => {
...
const getSparklesMessage = (blocks: CaseStudyData["content"]["tab_3_first_blocks"]) => {
  const lastBlock = blocks[blocks.length - 1];
  if (lastBlock && lastBlock.icon && lastBlock.icon.includes("sparkles")) {
    return lastBlock.title;
  }
  return null;
};
...
const PowerBiCaseStudy = ({ data }: PowerBiCaseStudyProps) => {
  ...
  const integrationPoints = mapIntegrationPoints(content.tab_3_first_blocks);
  const sparklesMessage = getSparklesMessage(content.tab_3_first_blocks);
  const documentationList = extractDocumentationList(content.tab_4_second_left_content);
  ...
  const enablementParagraphs = extractParagraphs(content.tab_4_second_left_content);
      ...
      <div className="bg-[#07111F]">
            ...
            <div className="grid items-center gap-12 lg:grid-cols-[6fr_5fr] lg:gap-16">
              ...
              <Reveal delay={180}>
                <div className={cn(cjCard, "overflow-hidden")}>
                  <SmartImage
                    src={data.image}
                    alt="Analytics command center displaying centralized Power BI dashboards"
                    width={1200}
                    height={912}
                    loading="eager"
                    fetchPriority="high"
        ...
        >
          ...
          <div className={cn(cjContainer, "relative z-10")}>
            ...
            <section id="solution-integration" className="focus:outline-none">
              <Reveal delay={60}>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {integrationPoints.map((k, i) => (
                  <Reveal key={k.text} delay={i * 40}>
                    <div className="flex h-full items-start gap-3 rounded-[18px] border border-white/[0.07] bg-[#102236]/60 px-4 py-3.5">
                      <DynamicIcon name={k.icon} aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-[#A9E7C2]" />
                      <span className="text-[14px] leading-[1.6] text-[#A8B8C7]">{k.text}</span>
                    </div>
                  </Reveal>
                ))}
              </div>
              ...
              {sparklesMessage && (
                <Reveal delay={300}>
                  <div className="mt-7 flex max-w-[680px] items-start gap-4 rounded-[20px] border border-[#69D6FF]/25 bg-[#69D6FF]/[0.06] px-5 py-4">
                    <Sparkles aria-hidden="true" className="mt-0.5 h-5 w-5 shrink-0 text-[#69D6FF]" />
                    <p className="text-left text-[15px] leading-[1.7] text-[#F7FAFC]">
                      {sparklesMessage}
                    </p>
                  </div>
            ...
            {/* Data lake + modeling */}
            <section id="solution-platform" className={cn("focus:outline-none", blockSpace)}>
              <Reveal delay={60}>
                <SectionTitle className="mb-6">Centralized Data Lake and Staging</SectionTitle>
              </Reveal>
            ...
            <section id="solution-security" className={cn("focus:outline-none", blockSpace)}>
              ...
              <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
                <div className="space-y-5">
                  {securityParagraphs.map((paragraph, i) => (
            ...
            {/* Performance and DAX */}
            <section id="solution-performance" className={cn("focus:outline-none", blockSpace)}>
              <Reveal delay={60}>
              ...
              <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
                ...
                <div className="space-y-5">
                  {daxParagraphs.map((paragraph, i) => (
                    <Reveal key={i} delay={140 + i * 40}>
                      <Body>{paragraph}</Body>
                    </Reveal>
                  ))}
                </div>
            ...
            {content.tab_3_sixth_heading && (
              <section id="solution-reporting" className={cn("focus:outline-none", blockSpace)}>
                <Reveal delay={60}>
                  <SectionTitle className="mb-6">{content.tab_3_sixth_heading}</SectionTitle>
                </Reveal>

                <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
                  <div className="space-y-5">
                    {reportingParagraphs.map((paragraph, i) => (
                      <Reveal key={i} delay={100 + i * 40}>
                        <Body>{paragraph}</Body>
                      </Reveal>
                    ))}
                  ...
                  <Reveal delay={140}>
                    <div className={cn(cjCard, "overflow-hidden")}>
                      <SmartImage
                        src={content.tab_3_sixth_image.url}
                        alt={content.tab_3_sixth_image.alt || ""}
                        width={content.tab_3_sixth_image.width}
                        height={content.tab_3_sixth_image.height}
                        loading="eager"
                        className="h-full w-full object-cover"
                      />
                    </div>
          ...
          <div className={cjContainer}>
            ...
            <section id="delivery-platform" className="focus:outline-none">
              ...
              <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-14">
                <Reveal delay={100}>
                  <TeamMap hub={content.tab_4_first_left_blocks[0].title} nodes={teamNodes} />
                </Reveal>
            ...
            <section id="delivery-enablement" className={cn("focus:outline-none", blockSpace)}>
              <Reveal delay={60}>
              ...
              <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-14">
                <div className="space-y-5">
                  {enablementParagraphs.map((paragraph, i) => (
                  ...
                  {documentationList.length > 0 && (
                    <Reveal delay={180}>
                      <ul className="grid gap-3 sm:grid-cols-2">
                        {documentationList.map((d) => (
                          <li
                            key={d}
                            className="flex items-start gap-3 rounded-[18px] border border-white/[0.07] bg-[#102236]/60 px-4 py-3.5"
                          >
                            <FileCheck aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0 text-[#A9E7C2]" />
                            <span className="text-[14px] leading-[1.6] text-[#A8B8C7]">{d}</span>
                          </li>
                        ))}
                      </ul>
                    </Reveal>
        ...
        >
          ...
          <div className={cn(cjContainer, "relative z-10")}>
            ...
            <section id="success-metrics" className="focus:outline-none">
              ...
              <div className="grid grid-cols-1 items-stretch gap-5 sm:grid-cols-2 lg:grid-cols-6 lg:gap-6">
                {successMetrics.map((m, i) => (
                  <Reveal
                    key={m.label}
                    delay={i * 80}
                    className={cn("h-full lg:col-span-2", i === 3 && "lg:col-start-2")}
                  >
                    <MetricCard item={m} />
                  </Reveal>
                ))}
            ...
            <section id="benefits" className={cn("focus:outline-none", blockSpace)}>
              ...
              <div className="grid grid-cols-1 items-stretch gap-4 sm:grid-cols-2 lg:grid-cols-6 sm:gap-5">
                {benefits.map((b, i) => (
                  <Reveal
                    key={b.text}
                    delay={i * 70}
        ...
        <section id="cta" className="relative overflow-hidden">
          {content.bottom_banner_image && content.bottom_banner_image.url && (
...
export default PowerBiCaseStudy;
