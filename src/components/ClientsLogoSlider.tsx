    ...
    >
      ...
      <div className="relative z-10">
        ...
        >
          ...
          <div
            className="absolute left-0 top-0 bottom-0 w-20 md:w-32 z-20 pointer-events-none"
            style={{
              background:
                "linear-gradient(to right, hsl(222 47% 8%) 0%, transparent 100%)",
            }}
          />

          {/* Right gradient fade - matches hero background */}
          <div
            className="absolute right-0 top-0 bottom-0 w-20 md:w-32 z-20 pointer-events-none"
            style={{
              background:
                "linear-gradient(to left, hsl(222 47% 8%) 0%, transparent 100%)",
            }}
          />

          {/* Scrolling Container */}
          <div
            className={`flex items-center relative z-10 py-3 ${isPaused ? "pause-animation" : ""}`}
            style={{
          ...
          >
            {allClients.map((client, index) => (
              <div
                key={index}
                className="flex-shrink-0 mx-8 md:mx-12 lg:mx-16 flex items-center justify-center group cursor-pointer"
              >
                {client.logo ? (
                  <img 
                    src={client.logo} 
